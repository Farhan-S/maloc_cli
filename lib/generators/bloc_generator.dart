class BlocGenerator {
  static Map<String, String> generate(
    ...
    return {
      'bloc':
          _generateBloc(snakeName, pascalName, camelName, withData: withData),
      'event': _generateEvent(snakeName, pascalName),
      'state':
          _generateState(snakeName, pascalName, camelName, withData: withData),
    };
  }
...
class ${pascalName}Bloc extends Bloc<${pascalName}Event, ${pascalName}State> {
  ...
  Future<void> _onGet${pascalName}(
    ...
    // For now, just emit a loaded state with sample data
    await Future.delayed(const Duration(seconds: 1));
    emit(const ${pascalName}Loaded(data: 'Sample data'));
  }
}
''';
    }
